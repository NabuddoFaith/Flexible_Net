 ...
 * Updated: Spatial clipping for Height Layers to prevent global computation
 */

// 1. AREA OF INTEREST
var region = ee.FeatureCollection("projects/ee-faithnabuddo/assets/rwenzori");
...
// 4. CANOPY HEIGHT MODEL (Spatial Clipping Fixed)
var dsm = ee.ImageCollection("JAXA/ALOS/AW3D30/V3_2")
  .filterBounds(region) // Constraint 1: Only load images in the study area
  .mosaic()
  .select('DSM')
  .clip(region);        // Constraint 2: Cut to the shapefile
...
Export.image.toDrive({
  ...
  scale: 10,
  region: region.geometry(), // Use .geometry() to ensure a valid export boundary
  fileFormat: 'GeoTIFF',
  maxPixels: 1e13
});
